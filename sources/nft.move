...
module overmind::NonFungibleToken {
    ...
    */
    fun init(ctx: &mut TxContext) {
        // create and transfer the minter cap object to the owner of the module
    ...
    */
    public fun mint_nft(
        ...
        let nft_price = 1000000000;

        // get coin's balance
        let coin_bal = coin::value<SUI>(payment_coin);

        // coin's balance must be greater than the price of the NFT
        assert!(coin_bal >= nft_price, EInsufficientPayment);
        ...
        let one_sui_coin = coin::take<SUI>(coin::balance_mut<SUI>(payment_coin), nft_price, ctx);
    ...
    */
    public fun combine_nfts(
        ...
        // create the name of the combined NFT
        let new_name = *&name1;
        string::append(&mut new_name, string::utf8(b" + "));
        string::append(&mut new_name, name2);

        // create the description of the combined NFT
        let new_description = string::utf8(b"Combined NFT of ");
        string::append(&mut new_description, name1);
        string::append(&mut new_description, string::utf8(b" and "));
        string::append(&mut new_description, name2);

        // create the new NFT object with the combined properties
        let combined_nft = NonFungibleToken {
            id: object::new(ctx),
            name: new_name,
        ...
        // notify observers of the newly combined NFT
        event::emit(NonFungibleTokenCombined {
            nft1_id: object::uid_to_inner(&id1),
            nft2_id: object::uid_to_inner(&id2),
            new_nft_id: object::id(&combined_nft),
        });
    ...
    */
    public fun withdraw_sales(
        ...
        // notify observers of the withdrawal action
        event::emit(SalesWithdrawn {
            amount: balance::value(&minter_cap.sales)
        });

        // withdraw sales and return the withdrawn coin
        coin::from_balance(balance::withdraw_all(&mut minter_cap.sales), ctx)
    }
